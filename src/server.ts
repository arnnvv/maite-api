...
app.post("/verify-token", async (req: Request, res: Response) => {
  ...
  try {
    const emailOrError: { email: string } | { error: string } =
      await getEmail(token);
    if ("error" in emailOrError) {
      res.status(401).json({ error: emailOrError.error });
      return;
    }
    res.json({ email: emailOrError.email });
  } catch (error) {
    res.status(401).json({ error: "Invalid token" });
  }
});
...
app.post("/send-otp", async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (userExists.length <= 0) {
    logWithColor(
      `ℹ️ No user found with email ${email}. Inserting new user record...`,
      "\x1b[33m",
    );
    try {
      await db.insert(users).values({ id: v4(), email });
      logWithColor(
        `✅ Successfully inserted user with email: ${email}`,
        "\x1b[32m",
      );
    } catch (insertionError) {
      logWithColor(
        `❌ Error inserting user ${email}: ${insertionError}`,
        "\x1b[31m",
      );
